...
module.exports.create = function(prefix, redis, options) {
    ...
    var changeLogLock = ChangeLogLock.create(prefix, redis, {logger: logger});
    ...
    var changeId = function(key) {
        return key.replace(changeKey(''), '');
    };
        ...
        lock(function(callback) { 
            ...
            redis.keys(redisKey, function(err, changes) {                   
                ...
                _.each(changes, function(change) {
                    logger.info("Clearing " + changeId(change).cyan);
                    multi.del(change);
                })
                multi.exec(callback);
            })
