...
*/
module.exports.create =	function(id, redis, options) {

	var options = options || {};
	var logger = options.logger || console;
	var prefix = options.prefix || 'changecontrol';			

	var changeLog = ChangeLog.create(prefix, redis, {logger: logger});		
	var changes = [];
	var abort;

	var add = function(changeId, script, options) {
		var change = Change.create(id + ':' + changeId, script, options);
		changes.push(change);
		return change;
	};
